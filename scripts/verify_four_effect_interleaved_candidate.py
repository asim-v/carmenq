"""Independently construct and verify the symmetric four-effect MPS branch."""
...
from carmenq.order_sensitive import interleaved_four_effect_lower_bound  # noqa: E402
from scripts.verify_compact_interleaved_candidate import (  # noqa: E402
    PAULIS,
    columns,
    prefix_tensors,
    syndrome,
...
def construction(weight: float) -> tuple[list[np.ndarray], np.ndarray, dict[str, object]]:
    point = interleaved_four_effect_lower_bound(weight)
    if point.strategy != "four_effect_mps":
        raise ValueError("the selected support direction uses the no-record branch")
    assert point.p is not None and point.theta is not None
    assert point.u_small is not None and point.u_large is not None
    assert point.priors is not None

    p_value = point.p
    theta = point.theta
    state_small = np.asarray(
        (np.sqrt(point.u_small), np.sqrt(1.0 - point.u_small)), dtype=complex
    )
    state_large = np.asarray(
        (np.sqrt(point.u_large), -np.sqrt(1.0 - point.u_large)), dtype=complex
    )
    ...
    tensors = prefix_tensors(list(states), point.priors)

    root_effect = np.diag((np.sqrt(p_value), np.sqrt(1.0 - p_value)))
    ...
    for label, vector in enumerate(effect_vectors):
        ...
        # B3 records x3 and the persistent qubit records the x2 decision.
        # Hence all four effect labels occupy orthogonal output states.
    ...
    metadata = {
        ...
        "p": point.p,
        "theta": point.theta,
        "u_small": point.u_small,
        "u_large": point.u_large,
    ...
    return tensors, effects, metadata


def evaluate(weight: float = 0.6) -> dict[str, object]:
    ...
    return {
        **metadata,
        ...
        "audit_residual": float(audit - point_value(metadata, "reported_audit")),
        "return_residual": float(returned - point_value(metadata, "reported_return")),
        "score_residual": float(score - point_value(metadata, "reported_score")),
    }


def point_value(metadata: dict[str, object], key: str) -> float:
