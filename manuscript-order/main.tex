...
for every $3/7<\lambda<1$, including balanced weight. A complete non-QND
numerical construction supplies an interior lower bound, but the exact
interleaved frontier remains open.
\end{abstract}
...
that conjunction; this is a dated collision assessment, not proof of priority.
...
$3/7<\lambda<1$. The dashed curve is an analytic achievable family, and the
diamond is an independently verified complete ternary-outcome strategy at
balanced weight. The unclosed vertical distance is not an error bar: it is the
remaining exact-frontier problem.}
\label{fig:support}
\end{figure*}

\section{Constructions and numerical falsification}
\label{sec:numerics}

A transparent two-parameter streamed construction gives exact lower bounds.
For $q,v\in[0,1]$,
\begin{align}
P_{\rm can}(q,v)
&=\frac12+qv\sqrt{1-v^2}-q(1-q)v^2,\label{eq:Pcan}\\
F_{\rm can}(q,v)
&=\frac14\left[\sqrt{1-(1-q^2)v^2}\right.\notag\\[-1mm]
&\hspace{9mm}\left.
+v\bigl(1-q+2\sqrt{q(1-q)}\bigr)\right]^2.
\label{eq:Fcan}
\end{align}
Only the first and third slots emit classical flags; the second and fourth
perform controlled qubit updates. Direct tensor contraction verifies
\eqref{eq:Pcan}--\eqref{eq:Fcan}. At balanced weight, this family reaches
$0.755437446229\ldots$.

That family is not the full frontier. A complete depth-four, local
ternary-outcome optimization over arbitrary complex non-QND instruments found
\begin{equation}
(\PA,\FR)=
(0.625754561820\ldots,,0.893143378814\ldots),
\end{equation}
with balanced support $0.759448970317\ldots$. The stored instrument contains
all local Kraus maps and terminal effects. An independent NumPy contraction
checks every local completeness relation, solves the qubit AUDIT dual, and
evaluates RETURN from unmodified singular values. The maximum completeness
residual is $1.1\times10^{-15}$. Its rank-four spectral tail is
$0.452260604\ldots$, below the theorem's normalized allowance
$2(1-\PA)=0.748490876\ldots$.

The numerics serve two roles. They provide a physical interior lower bound and
falsify an attractive low-parameter ansatz. They do not certify global
optimality. A one-leaf tensor-rank relaxation reaches
$0.759802783851\ldots$, only $3.54\times10^{-4}$ above the best complete
strategy, but local completeness is exactly the condition omitted by that
relaxation. Treating the small difference as an uncertainty interval would be
incorrect.
...
\end{align}
Closing this gap requires either a stronger complete causal instrument or an
upper certificate that uses local completeness more efficiently than the
union bound. The optimal coefficient in the near-endpoint square-root law and
extensions to partially ranked block sequences are also open.
...
\end{quote}
reproduce the software checks, balanced construction, plotted bounds, and
this PDF. Numerical searches are explicitly lower-bound generators. The
proved claims are \eqref{eq:productlaw}, \eqref{eq:lineartail},
\eqref{eq:returnbound}, and the interval \eqref{eq:strictgap}; no exact
interleaved interior curve is claimed.
