...
\paragraph{AUDIT.}
The verifier measures every $R_i$ in the $Z$ basis, obtaining independent uniform bits $X_i$, and uses $M$ to decode a guess for
\[
...
After refining all classical random seeds and outcomes, every leaf transcript $k=(k_1,\ldots,k_n)$ induces a trace-nonincreasing CP map that factorises over slots. For computational-basis inputs $x=(x_1,\ldots,x_n)$, its likelihood is
\begin{equation}
...
\label{eq:bias}
\end{equation}
Either sign may be chosen when $\delta_i=0$.
