...
does **not** close the target value (0.758).  Its main value is that it replaces
the vague requirement that all conditioned states "come from one instrument"
by a family of explicit trace-norm inequalities, and it identifies exactly why
the three canonical inequalities are not yet sufficient.

## Harmonic contraction theorem

Let (G) be a finite abelian group and let
({Phi_y}_{y\in G}) be a quantum instrument, so each (Phi_y) is
completely positive and the flagged channel
...
outcomes.  The proof uses only a single common instrument.  Applying a
different channel to every input would not justify it.
...
disjunction into finitely many convex outer problems.  For (G=\mathbb Z_2^2),
Parseval supplies the additional joint inequality
...
The implemented spherical cover uses normalised cube-face charts.  On a face,
write a direction as (f(u,v)=(1,u,v)/\|(1,u,v)\|).  The derivative norm of
(f) is at most one.  A square of half-width (1/N) is therefore contained in
a cap with chord radius at most (sqrt2/N), hence covering cosine at least
(1-1/N^2).  This supplies a proved cover rather than a sampled set of
directions.

## Interior benchmark

The benchmark fixes terminal effect weights
((0.92,0.64,0.44,0)), prefix order (a_0\geq a_1\geq a_2\geq a_3), and
(lambda=0.55).  An independently generated outer superlevel box is
...
moment outer gives (0.75115255); an independent SCS run gives (0.75116220).
Both are below (0.758), but they cover only that regime.

The fully vector-active regime remains the obstruction.  A reduced SOCP with
16 planar sectors and 384 proved spherical caps solved 6,144 cells and returned
(0.76293706).  At its worst point, the three character contractions are
nearly saturated, yet the flagged channel expands
(ho_2-\rho_3): its output norm is (0.32304) while its input norm is
(0.22977).  Adding that pairwise contraction moves the violation first to
(ho_1-2\rho_2), then to (ho_0-2\rho_3).  Hence the three group
...
counterexample.  The remaining mathematical task is to replace an adaptive
list of scalar coefficient directions by either a complete finite criterion
for the four-input qubit operator system or a verified spatial cover of those
directions.  Complete positivity, rather than the terminal Helstrom geometry,
is now the identified bottleneck.

## Reproducibility map

The large moment implementation is in
`scratch/d2_frontier/choi_moment_reduced_upper.py`.  The small parameterised
SOCP is in `scratch/d2_frontier/fourier_behavior_upper.py`; its cap-cover driver
is `scratch/d2_frontier/fourier_behavior_cap_cover.py`.  The compact canonical
result is `scratch/d2_frontier/fourier_interior_summary_l055.json`; the full
6,144-cell JSON is deliberately regenerated rather than committed.  These
files report solver-conditional conic bounds.  No claim of interval-verified
numerics is made.
